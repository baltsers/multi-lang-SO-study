...
JOERNPATH = "/pool/home/hcai/bin/joern/joern-cli/"
...
def initAPI(repo_name):
    ...
    '''
    logging.info(PY_API_O)
    logging.info(PY_API_M)
    logging.info(C_API_O)
    logging.info(C_API_M)
    '''
...
def checkInAPI(ident_set,version):
    if version =="modified":
        xapi = ident_set & C_API_M
        if len(xapi) !=0:
            logging.warning("there is an API below in modified")
            print (xapi)
            return True, xapi

        xapi = ident_set & PY_API_M
        if len(xapi) !=0 :
            print (xapi)
            return True,xapi
    ...
    # this mean: version == "origin"
        ...
        xapi = ident_set & C_API_O
        if len(xapi) !=0:
            logging.warning("there is an API below in modified")
            print (xapi)
            return True, xapi

        xapi = ident_set & PY_API_O
        if len(xapi) !=0 :
            print (xapi)
            return True,xapi
...
def getLineIdent(line_n, code, version):
    if version == "modified":
        if line_n in Line_Ident_M:
            ...
            cmd="echo \"%s\" | python3.6  -m tokenize 2>/dev/null | grep NAME | awk '{print $3}'" % code
            logging.info("shell command to execute: %s" % cmd)
            line_idents=set(os.popen(cmd).read().replace("'","").rstrip().split('\n'))-set(keyword.kwlist)-set(C_KEYWORDS)
            tmp=list(line_idents)
            for idt in tmp:
                if len(idt)<1:
                    line_idents.remove(idt)
            logging.info("line:%s, identifiers extracted:%s" % (code, line_idents))
...
def fsearchCore(start, S, lines,sha,repo_name,file_name,version):
    ...
    linen_id = getLineIdent(line_n,lines[line_n],version)
    if len(S & linen_id) != 0:
        logging.warning("found one inter-language during forward search: S=%s, API containing line: no. %d; line itself %s; identifier set %s" % (S, line_n, lines[line_n], linen_id))
        addToCsv(sha, repo_name,"inter",version+'-'+file_name+'-'+str( S & linen_id)+'-'+str(line_n)+'-'+str(xapi))
        return True
    else:
...
def bsearchCore(start, S, lines,sha,repo_name,file_name,version):
    ...
    linen_id = getLineIdent(line_n,lines[line_n],version)
    if len(S & linen_id) != 0:
        logging.warning("found one inter-language during backward search: S=%s, API containing line: no. %d; line itself %s; identifier set %s" % (S, line_n, lines[line_n], linen_id))
        addToCsv(sha, repo_name,"inter",version+'-'+file_name+'-'+str( S & linen_id)+'-'+str(line_n)+'-'+str(xapi))
        return True
    else:
...
def compareCodeChangeidentifier(sha,repo_name,codechange_record):
    ...
    for record in codechange_record:
        if record[2] == "modified":
            #S = set([])
            logging.warning("changed code in modified: line no. %d; line itself: %s" % (record[1], lines[record[1]]))
            S = getLineIdent(record[1],lines[record[1]],"modified")
            if len(S)<1:
                logging.warning("the changed line does not have any identifier; skipped.")
                continue
    ...
    for record in codechange_record:
        if record[2] == "origin":
            #S = set([])
            logging.warning("changed code in origin: line no. %d; line itself: %s" % (record[1], lines[record[1]]))
            S = getLineIdent(record[1],lines[record[1]],"origin")
            if len(S)<1:
                logging.warning("the changed line does not have any identifier; skipped.")
                continue
